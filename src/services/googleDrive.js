...
/**
 * Loetleb Google Drive'ist failid, mille nimi sisaldab ".nm", ".nmc" (või vana ".noodimeister").
 * @param {string} accessToken
...
export async function listNoodimeisterFiles(accessToken, options = {}) {
  ...
  let q = "trashed = false and (name contains '.nm' or name contains '.nmc' or name contains '.noodimeister')";
  if (folderId) {
...
/**
 * Loetleb Google Drive'ist failid, mis on kasutajaga jagatud (shared with me) ja mille nimi sisaldab ".nm", ".nmc" (või vana ".noodimeister").
 * @param {string} accessToken
 * @returns {Promise<Array<{ id, name, modifiedTime, createdTime }>>}
 */
