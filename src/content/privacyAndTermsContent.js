...
export const PRIVACY_BODY = {
  et: [
    ...
    '9. Google sisselogimine ja registreerimine (etapid)',
    'Etapp 1: kasutaja vajutab lehel Logi sisse või Registreeru nuppu „Google”. Etapp 2: Google autentib kasutaja Google’i enda lehel. Etapp 3: Noodimeister saab Google\'ilt autentimistulemuse ning loob/uuendab rakenduses konto kirje (provider + email).',
    'Etapp 4: kui kasutaja kasutab Google Drive salvestust/laadimist, küsitakse eraldi vastavaid õigusi (scopes) failide haldamiseks kasutaja enda Drive\'is. Etapp 5: kasutaja saab õigusi tühistada Google konto Security/Permissions lehel ja rakendusest välja logides eemaldatakse kohalikud seansiandmed.',
    '10. Miks Google kasutajaandmeid küsitakse',
    'Google profiili põhiandmeid (nt e-post, nimi) kasutatakse ainult sisselogimiseks, konto sidumiseks ja kasutajakogemuse kuvamiseks. Google Drive õigusi kasutatakse ainult kasutaja algatatud toiminguteks (pilve salvesta, laadi pilvest, faili nime/kausta valik). Noodimeister ei müü ega rendi neid andmeid kolmandatele osapooltele.',
    '11. Andmete kustutamine ja õiguste tühistamine',
    'Kasutaja saab igal ajal: (a) rakendusest välja logida, (b) eemaldada Noodimeistri ligipääsu Google konto õigustest, (c) taotleda kontoandmete kustutamist e-posti teel aadressil info@la-stravaganza.com. Pilvefailid (Google Drive/OneDrive) jäävad kasutaja pilvekontole, kuni kasutaja need ise kustutab.',
  ],
  en: [
    ...
    '9. Google sign-in and registration flow',
    'Step 1: the user clicks the Google button on Sign in or Register. Step 2: Google authenticates the user on Google\'s own page. Step 3: Noodimeister receives the authentication result and creates/updates the app account identity (provider + email).',
    'Step 4: when the user chooses Google Drive save/load features, the app requests the required Drive permissions (scopes) for the user\'s own Drive. Step 5: the user can revoke those permissions at any time from Google Account Security/Permissions, and sign-out removes local session data from the app.',
    '10. Why Google user data is requested',
    'Basic Google profile data (for example email and name) is used only for authentication, account linking, and account display inside the app. Google Drive permissions are used only for user-initiated actions (save to cloud, load from cloud, select folder/file). Noodimeister does not sell or rent this data to third parties.',
    '11. Data deletion and revocation',
    'Users may at any time: (a) sign out from the app, (b) remove Noodimeister access in Google account permissions, and (c) request deletion of account-side data by contacting info@la-stravaganza.com. Cloud files in Google Drive/OneDrive remain under the user account until the user deletes them.',
  ],
  fi: [
    ...
    '9. Google-kirjautumisen ja rekisteröinnin vaiheet',
    'Vaihe 1: käyttäjä painaa Google-painiketta kirjautumisessa tai rekisteröinnissä. Vaihe 2: Google tunnistaa käyttäjän omalla sivullaan. Vaihe 3: Noodimeister saa tunnistustuloksen ja luo/päivittää tili-identiteetin (provider + email).',
    'Vaihe 4: kun käyttäjä käyttää Google Drive -tallennusta/latausta, sovellus pyytää tarvittavat käyttöoikeudet käyttäjän omaan Driveen. Vaihe 5: käyttäjä voi perua oikeudet Google-tilin turvallisuusasetuksista; uloskirjautuessa paikallinen istuntodata poistetaan sovelluksesta.',
  ],
};
...
export const TERMS_BODY = {
  et: [
    ...
    'Google sisselogimise/registreerimise kasutamisel nõustud, et autentimine toimub Google OAuth voos ning rakendus kasutab saadud ligipääsu vaid kasutaja algatatud toiminguteks. Sul on õigus ligipääs igal ajal Google konto seadetes tagasi võtta.',
  ...
  en: [
    ...
    'When using Google sign-in/registration, you agree that authentication is performed through Google OAuth and that granted access is used only for user-initiated features in the app. You can revoke this access at any time in Google account settings.',
    ...
    'Google-kirjautumista käytettäessä tunnistautuminen tapahtuu Google OAuth -prosessilla, ja käyttöoikeuksia käytetään vain käyttäjän pyytämiin toimintoihin. Käyttäjä voi perua oikeudet milloin tahansa Google-tilin asetuksista.',
